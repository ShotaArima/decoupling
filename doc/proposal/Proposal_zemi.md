...
| latent を分ければ解釈可能になるように見える | latent split だけでは decoder 内で混ざるため、4変数への分解が必要だと明示する |
...
### 3.6 4変数への分解 + centering は latent split より説明しやすい

2-Exp-28 では、元論文に近い latent split と、本研究の 4変数への分解を同じ `series_mean` residual 上で比較した。

予測補正では、centering ありの4変数モデル 系が最も良かった。
...
高残差 top10 でも、centering ありの4変数モデルは改善した。
...
hour profile でも、centering ありの4変数モデル が安定した。
...
4変数への分解と centering により、補正性能と成分解釈を同時に評価できる。
```
...
- latent split ではなく 4変数への分解を主張の中心にする。
...
- 2-Exp-28 では 4変数への分解と centering が latent split よりよい補正と hour profile 対応を示した。
- したがって、本研究の主張は latent split ではなく 4変数への分解 に置く。

### Slide 10: 現時点の論文ライン

```text
Synthetic:
成分が存在する条件で 4変数への分解と centering は成分を回復できる。
...
| 優先度 | 作業 | 目的 |
|---|---|---|
| 高 | Method 節を 4変数への分解 中心に書き直す | 主張の中心を固定する |
| 高 | Related Work を local/global latent から residual 4変数モデル へつなぐ | 既存研究との差分を明確にする |
