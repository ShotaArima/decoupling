# 2-Exp-28: Latent Split vs Four-Factor Modeling

## 目的

2-Exp-28 は、元論文型の latent split と、本研究の主提案である 4変数への分解を同じ `series_mean` residual 条件で直接比較する実験である。
...
4変数への分解と centering constraints は
...
| `output_decomp_no_center` | `output_decomposition` | 4変数への分解のみ。centering なしの負例 |
...
理由は、この実験の目的が補正性能を最大化することではなく、同じ residual target 上で latent split と 4変数への分解の違いを見ることだからである。
...
全モデルが baseline より低い corrected MAE を出したが、最も良かったのは 4変数モデル系だった。
...
Exp-28 では、同じ residual target 上で 4変数への分解と centering が latent split 系より良い corrected MAE を示した。
...
この結果は、4変数への分解の価値を「平均 MAE」だけでなく「外れケース補正」としても説明できることを示す。

### hour profile と成分 ablation

Hour profile でも、centering ありの4変数モデル が最も安定した。
...
成分 ablation では、centering ありの4変数モデルの hour 成分が最も明確に効いていた。
...
一方で、centering ありの4変数モデル でも corrected bias は残った。
...
| `hour_component_residual_profile_corr` | 4変数への分解の hour component が residual hour profile と対応するか |
...
4変数モデル系では、`component_ablation_without_*_mae_delta` と component profile を見る。
...
一方、4変数への分解と centering は、出力成分そのものに構造を課すため、
補正性能と成分解釈を同時に評価できる。
```
...
```text
4変数への分解は raw model の段階で latent split より良い補正と hour profile 対応を示した。
