...
この結果は、真の成分がある controlled setting では、出力分解と平均ゼロ制約によって各成分をほぼ正しく回収できることを示している。
...
> 残差に構造が存在し、成分の平均ゼロ条件が妥当なとき、出力分解付きの residual representation は成分を回収できる。実データではこの条件を満たす subset で補正効果を検証する必要がある。
