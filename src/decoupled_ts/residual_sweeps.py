...
def _mean_std(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    group_keys = [key for key in ("scenario", "name") if any(key in row for row in rows)]
    ...
    metric_keys = sorted(
        {
            ...
            if key not in {"name", "seed", "scenario"} and isinstance(value, int | float)
        }
    )
    summary = []
    for group in groups:
        subset = [row for row in rows if tuple(row.get(key, "") for key in group_keys) == group]
        out: dict[str, Any] = {key: value for key, value in zip(group_keys, group, strict=True)}
        out["runs"] = len(subset)
...
def run_residual_sweep(config_path: str) -> dict[str, Any]:
    ...
    for scenario in scenarios:
        ...
        scenario_dir = _safe_name(scenario_name)
        for seed in seeds:
            config = deepcopy(base_config)
            config["seed"] = seed
            config = _deep_update(config, deepcopy(scenario.get("overrides", {})))
            config["train"]["output_dir"] = str(root_out / scenario_dir / f"seed_{seed}")
            config_path_for_seed = root_out / f"{scenario_dir}_seed_{seed}_config.json"
            write_json(config_path_for_seed, config)
            logger.info("Running scenario=%s seed=%d output_dir=%s", scenario_name, seed, config["train"]["output_dir"])
            result = run_residual_experiments(str(config_path_for_seed))
            for row in result["results"]:
                prefix = {"scenario": scenario_name} if has_scenarios else {}
                all_rows.append({**prefix, "seed": seed, **row})
